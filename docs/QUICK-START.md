...
In this tutorial, we are going to learn how Urbanite can easily help with visualizing a simple dataset using a Vega-Lite barchart. 

After initializing Urbanite, we can see a blank canvas like this:
...
Urbanite handles the data-flow and `Vega-Lite` has access to the DataFrame outputed by the previous box. After hitting run, we can see that a barchart was created:

![Final result](images/final_result.png?raw=true)

Congratulations! You created your first data-flow using Urbanite :)
